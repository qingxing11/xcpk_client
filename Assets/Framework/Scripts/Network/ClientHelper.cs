...
public class ClientHelper
{
    ...
    /// <summary>
    /// 0:protobuf-net,1:json
    /// </summary>
    private static int serializerUtil = 0;
    public static void InitConnect(string host, int port, int serializerUtil, OnRpcDataDelegate AddReceiveMission)
    {
        msgList.Clear();
        ClientHelper.serializerUtil = serializerUtil;
	...
	public static void Disconnect()
    {
        if (tcp != null)
        {
            ...
            {
                Debug.Log("错误====>tcp.GetStream().Close():"+e);
            }
            ...
            {
                Debug.Log("错误====> tcp.Client.Close()" + e);
            }
            ...
            {
                Debug.Log("错误====>tcp.Close()" + e);
            }
            Debug.Log("disconnect结束");
        }

    }

    private static void Disconnect(bool reuse)
    {
        try
        {
            if (tcp.Client.Connected)
            {
                ...
                tcp.Client.Disconnect(reuse); 
            }
        }
        catch (Exception e)
        {
            Debug.Log("错误====>tcp.Client.Disconnect():"+e);
        }
    }
    ...
    public static bool sendRequest(Request request)
    {
        ...
        switch (serializerUtil)
        {
            ...
            case 1://protobuf_net
                msg = MySerializerUtil.SerializerToProtobufNet(request);
                break;

            case 2://json
                string msgStr = JsonUtility.ToJson(request);
                msg = msgStr.GetBytes();
                break;
        ...
        return sendRequest(msg, request.msgType);
    }
    ...
    public static bool sendRequest(byte[] msg, int msgType)
    {
        ...
        bool isSuccess = Write(packetBytes);
        if (isSuccess)
        {
    ...
    private static bool Write(byte[] msg)
    {
        try
        {
        ...
        {
            string error = "发送数据错误" + ex;
            //string error = "发送数据错误" + ex.StackTrace + ex.Message + " " + ex.StackTrace;
